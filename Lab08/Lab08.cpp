...
void HalfSpace(Triangle triangle)
{
    float e1, e2, e3;
    int x, y;
    int xMin = std::min(std::min(triangle.v1.x, triangle.v2.x), triangle.v3.x);
    int xMax = std::max(std::max(triangle.v1.x, triangle.v2.x), triangle.v3.x);
    int yMin = std::min(std::min(triangle.v1.y, triangle.v2.y), triangle.v3.y);
    int yMax = std::max(std::max(triangle.v1.y, triangle.v2.y), triangle.v3.y);
    for (y = yMin; y <= yMax; y++)
    {
        for (x = xMin; x <= xMax; x++)
        {
...
void Midpoint(Vec2 v0, Vec2 v1)
{
    ...
    {
        if (x0 > x1) { MidpointLow(v1, v0, 1); }
        else { MidpointLow(v0, v1, 0); }
    }
    else
    {
        if (y0 > y1) { MidpointHigh(v1, v0, 1); }
        else { MidpointHigh(v0, v1, 0); }
    }
}
