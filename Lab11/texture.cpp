...
void SetupRC()
{
	...
	cv::Mat floorImage = cv::imread("../OpenCVTexture/floor.jpg"); // 利用openCV讀取圖片檔案
	if (floorImage.empty()) {
	...
	// 下面要設定剩下的三種材質
	cv::Mat block4Image = cv::imread("../OpenCVTexture/Block4.jpg"); // 利用openCV讀取圖片檔案
	if (block4Image.empty()) {
		std::cout << "Block4 empty\n";
	}
	else {
		// 將讀取進來的圖片檔案當作材質存進textures中
		cv::flip(block4Image, block4Image, 0);
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, block4Image.cols, block4Image.rows, 0, GL_BGR_EXT, GL_UNSIGNED_BYTE, block4Image.ptr());
	}

	cv::Mat block5Image = cv::imread("../OpenCVTexture/Block5.jpg"); // 利用openCV讀取圖片檔案
	if (block5Image.empty()) {
		std::cout << "Block5 empty\n";
	}
	else {
		// 將讀取進來的圖片檔案當作材質存進textures中
		cv::flip(block5Image, block5Image, 0);
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, block5Image.cols, block5Image.rows, 0, GL_BGR_EXT, GL_UNSIGNED_BYTE, block5Image.ptr());
	}

	cv::Mat block6Image = cv::imread("../OpenCVTexture/Block6.jpg"); // 利用openCV讀取圖片檔案
	if (block6Image.empty()) {
		std::cout << "Block6 empty\n";
	}
	else {
		// 將讀取進來的圖片檔案當作材質存進textures中
		cv::flip(block6Image, block6Image, 0);
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, block6Image.cols, block6Image.rows, 0, GL_BGR_EXT, GL_UNSIGNED_BYTE, block6Image.ptr());
	}
}
