...
int main(int argc, char** argv)
{
    ...
    glutCreateWindow("2D Grid animated gradient");
...
void RandomColor(Vec2& vec)
{
    vec.r = rand() % 256 / 250.f;
    vec.g = rand() % 256 / 250.f;
    vec.b = rand() % 256 / 250.f;
    //rgb(vec);
}
void AnimatePixels(const std::vector<Vec2>* pixels)
{
    Color3f color;
    ...
    {
        if (i > pIndex) { break; }
        color.r = pixels->at(i).r;
        color.g = pixels->at(i).g;
        color.b = pixels->at(i).b;
        SetPixel(pixels->at(i).x, pixels->at(i).y, color);
    }
    pIndex += 2;
}
...
void Crow(Vec2 vList[], int n, std::vector<Vec2>* pixels)
{
    int iMin = 0;
    for (int i = 1; i < n; i++)
    {
    ...
    ScanY(vList, n, iMin, pixels);
    delete[] vList;
}
void Crow(const Rect& rect, std::vector<Vec2>* pixels)
{
    AssignVList(rect);
    Crow(vList, 4, pixels);
}
void Crow(const Triangle& triangle, std::vector<Vec2>* pixels)
{
    AssignVList(triangle);
    Crow(vList, 3, pixels);
}
void Crow(std::vector<Vec2> poly, std::vector<Vec2>* pixels)
{
    if (poly.size() < 3) { return; } // less than 3 can't fill color
    AssignVList(poly);
    Crow(vList, poly.size(), pixels);
}
...
void MyDisplay(void)
{
    ...
    else
    {
        ...
        else if (MODE == POLY_MAX)
        {
            for (int i = 0; i < poly.size(); i++)
            {
                glColor3f(poly.at(i).r, poly.at(i).g, poly.at(i).b);
    ...
    {
        // midpoint algorithm
        // Halfspace algorithm (with animation)
        // Crow's algorithm
        if (MODE == RECT)
        {
            Midpoint(rect);
...
void MyMouse(int button, int state, int x, int y)
{
    ...
    if (debug) { printf("Mouse (%d, %d) --> origin (%.2f, %.2f) --> grid (%.2f, %.2f)\n", x, y, xx, yy, vec.x, vec.y); }

    // reset index and pixel vector for re-draw
    pIndex = 0;
    pixels.clear();
    if (MODE == TRIANGLE)
    {
